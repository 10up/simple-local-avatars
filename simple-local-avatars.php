...
class Simple_Local_Avatars {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_assign_simple_local_avatar_media', array(
			$this,
			'ajax_assign_simple_local_avatar_media'
		) );
	...
	public function admin_init() {
		...
		add_settings_field( 'simple-local-avatars-only', __( 'Local Avatars Only', 'simple-local-avatars' ), array(
			$this,
		...
		add_settings_field( 'simple-local-avatars-caps', __( 'Local Upload Permissions', 'simple-local-avatars' ), array(
			$this,
			'avatar_settings_field'
		), 'discussion', 'avatars', array(
			'key'  => 'caps',
	...
	public function edit_user_profile( $profileuser ) {
		...
		<table class="form-table">
			<tr>
				<th scope="row"><label
					for="simple-local-avatar"><?php _e( 'Upload Avatar', 'simple-local-avatars' ); ?></label></th>
				...
				<td>
					...
					if ( $upload_rights ) {
						...
						<?php } ?>
						<p>
							<?php if ( current_user_can( 'upload_files' ) && did_action( 'wp_enqueue_media' ) ) : ?><a
							href="#" class="button hide-if-no-js"
							id="simple-local-avatar-media"><?php _e( 'Choose from Media Library', 'simple-local-avatars' ); ?></a> &nbsp;<?php endif; ?>
							<a href="<?php echo $remove_url; ?>" class="button item-delete submitdelete deletion"
							   id="simple-local-avatar-remove"<?php if ( empty( $profileuser->simple_local_avatar ) ) {
								echo ' style="display:none;"';
							} ?>><?php _e( 'Delete local avatar', 'simple-local-avatars' ); ?></a>
						</p>
						<?php
					} else {
			...
			<tr>
				...
				<td colspan="2">
					<fieldset
					id="simple-local-avatar-ratings" <?php disabled( empty( $profileuser->simple_local_avatar ) ); ?>>
						...
						?>
						<p
						class="description"><?php _e( 'If the local avatar is inappropriate for this site, Gravatar will be attempted.' ); ?></p>
					</fieldset>
				</td>
			</tr>
		</table>
